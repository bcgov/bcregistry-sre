  ...
  <UContainer class="py-10 space-y-8">
    <div class="border-b border-gray-200 dark:border-gray-800 pb-4">
      ...
      <p class="text-gray-500 mt-1">
        Simulate application events, inspect live PII cleansing frameworks, and
        view real-time operations queries.
      </p>
    </div>
      ...
      <UCard>
        ...
        <div class="space-y-4">
          <p class="text-sm text-gray-600 dark:text-gray-400">
            Interact with the triggers below to generate contextual diagnostic
            outputs:
          </p>
          ...
          >
            <div
              class="flex items-center space-x-2 text-red-800 dark:text-red-400 font-medium text-sm"
            >
              <span>⚠️ Critical Unhandled Failure Encountered</span>
            </div>
            <p class="text-xs text-red-700 dark:text-red-300 mt-1">
              The system safely cached this instance. Please provide this
              support ticket signature to administration:
            </p>
            <div
              class="font-mono text-xl font-bold mt-2 tracking-wider text-red-900 dark:text-red-200"
            >
              {{ activeSupportCode }}
            </div>
          </div>
      ...
      <UCard>
        ...
        <div class="space-y-4">
          <div>
            ...
            <div
              class="font-mono text-sm bg-gray-100 dark:bg-gray-800 p-2 rounded mt-1 overflow-x-auto"
            >
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-gray-500">
              Execute this block inside the Google Log Explorer dashboard to map
              the chronological history of this individual session:
            </p>

            <div class="relative group">
              <pre
                class="bg-gray-900 text-emerald-400 p-4 rounded-lg font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed shadow-inner border border-gray-800"
                >{{ logQuery }}</pre
              >
            </div>
          </div>
...
const logQuery = computed(() => {
  const shortCode = activeSupportCode.value
    ? activeSupportCode.value.replace("ERR-", "")
    : "XXXXXXXX";
...
const triggerNormalLog = () => {
  $log.info("Citizen advanced to step 3 of application process", {
    currentStep: 3,
    formType: "Archaeological Permitting",
  });
};
...
const triggerVueError = () => {
  ...
  throw new Error(
    "Database integrity check failed during client form transmission.",
  );
};
...
const triggerPromiseRejection = () => {
  ...
  Promise.reject(
    new Error(
      "Network response timed out on critical synchronization pipeline.",
    ),
  );
};
...
const triggerShowErrorPage = () => {
  const shortCode =
    activeSupportCode.value ||
    `ERR-${(activeSessionId.value || sessionStorage.getItem("ops_session_id") || "").split("-")[0].toUpperCase()}`;
  activeSupportCode.value = shortCode;

  $log.error("Forced application exception from dashboard.", {
